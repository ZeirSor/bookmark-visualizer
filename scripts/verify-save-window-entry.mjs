...
const distSavePath = resolve(root, "dist/save.html");
...
function checkManifest(path, label) {
  ...
  assert(
    ...
    `${label}: action.default_popup should be omitted so chrome.action.onClicked can open save.html`
  );
  ...
  assert(
    ...
    `${label}: save window entry should not declare global host_permissions`
  );
  assert(
    !manifest.content_scripts,
    `${label}: save window entry should not declare global content_scripts`
  );
}
...
assert(
  ...
  "service worker: toolbar click should register chrome.action.onClicked"
);
assert(
  serviceWorker.includes("openSaveWindowForTab"),
  "service worker: save window opener should be wired"
);
assert(
...
assert(
  ...
  "shortcut access: legacy quick-save command bundle should remain available"
);
assert(
...
assert(viteConfig.includes("save.html"), "vite config: save.html is not configured as a build input");
assert(viteConfig.includes("popup.html"), "vite config: popup.html fallback is not configured as a build input");
assert(
...
if (
  ...
  assert(existsSync(distSavePath), "dist: save.html is missing");
  assert(existsSync(distPopupPath), "dist: popup.html fallback is missing");
  ...
  if (existsSync(distServiceWorkerPath)) {
    ...
    assert(
      existsSync(distQuickSaveContentPath),
      "dist: legacy quick-save-content.js bundle should remain available"
    );
    assert(
...
if (errors.length > 0) {
  console.error("Save window entry verification failed:");
...
console.log("Save window entry verification passed.");
