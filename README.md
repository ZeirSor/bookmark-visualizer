...
Bookmark Visualizer is a Manifest V3 browser extension for people with large bookmark collections. Click the extension toolbar icon to open a spacious bookmark workspace: a folder tree on the left, bookmark cards on the right, and search plus management actions around the edges.
...
The toolbar icon is the launch entry. Bookmark Visualizer opens as a full-page extension tab and leaves the browser's default new tab page unchanged.
...
5. Pin the extension if desired, then click the toolbar icon to open Bookmark Visualizer.
