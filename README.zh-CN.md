...
浏览器工具栏图标会打开独立的 `save.html` 扩展小窗口。完整管理工作台由 `index.html` 承载，`popup.html` 仍作为 fallback 入口参与构建。默认情况下，Bookmark Visualizer 保留浏览器原生新标签页；用户可以在设置中开启可选的 New Tab 接管，开启后新建标签页会跳转到 `newtab.html`。
...
- 通过工具栏图标或 `Ctrl+Shift+S` 打开保存小窗口，把当前网页保存为浏览器原生书签。
- 在 popup 的“保存”Tab 中搜索文件夹、选择保存位置、新建文件夹、填写备注并保存页面。
- 可从 popup 打开完整工作台，继续进行深度书签管理。
- 可选接管浏览器新标签页，提供搜索优先的书签入口页。
- New Tab 仪表盘和管理页卡片优先使用本地缓存的网站 favicon，不可用时显示本地字母 fallback。
- 支持配置 New Tab 搜索引擎、搜索分类、布局模式、每行快捷入口数量、最近活动和存储使用信息显示。

## 预览

首个公开版本前应补充稳定维护的截图或演示 GIF。未来加入演示资源时，应放在 `docs/assets/` 下，并在本段引用。
...
- 在普通网页点击工具栏图标或按 `Ctrl+Shift+S`，打开保存小窗口的“保存”Tab。
...
src/
  ...
  background/          MV3 service worker、命令、save-window 路由、quick-save 和 new-tab redirect 处理
  components/          跨页面复用的共享 UI 组件
  domain/              书签、文件夹、活动记录和表格视图等领域模型
  features/            功能模块：bookmarks、popup/save-window 支撑、quick-save、newtab、favicon、settings、metadata、search
  lib/chrome/          Chrome API adapter 和可 mock 的浏览器集成层
  newtab/              可选 New Tab 页面入口和应用装配
  popup/               Save / Manage / Settings 共享 UI，供 popup fallback 和保存小窗口复用
  save-window/         独立保存小窗口页面入口和应用装配
...
popup.html             工具栏 popup fallback 入口
save.html              独立保存小窗口入口
...
- 摘要抓取仍属于后续能力；当前保存小窗口路径不请求全局 `http://*/*` 或 `https://*/*` 站点访问权限。
- 组件级 UI 测试。
- 首个公开版本打包和稳定截图维护。
