...
5. 可按需固定扩展图标，然后点击工具栏图标打开 Save / Manage / Settings popup。
...
- 点击工具栏图标或按 `Ctrl+Shift+S`，打开圆角的工具栏 popup。
- 检查自动识别到的标题、URL、favicon、预览图和保存位置。
- 通过搜索文件夹、内联文件夹树或最近使用文件夹选择保存目标。
- 如有需要，可以在选择保存位置时新建文件夹。
- 填写备注后，将当前页面保存为浏览器原生书签。
- 可从“管理”Tab 打开完整工作台。浏览器内部页等受限页面可保存为 URL 引用，但不执行 metadata 注入。
