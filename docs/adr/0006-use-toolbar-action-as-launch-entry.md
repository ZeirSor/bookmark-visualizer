...
当前工具栏点击由 `action.default_popup` 打开 `popup.html`，用于保存当前网页；完整 `index.html` 工作台由 popup 顶部外链按钮或“管理”Tab 打开。service worker 不再把 `chrome.action.onClicked` 作为工具栏主入口。
...
- 快速保存由 popup 承载，完整管理仍然保留完整页面空间，适合拖拽、搜索、行内编辑和操作日志。
