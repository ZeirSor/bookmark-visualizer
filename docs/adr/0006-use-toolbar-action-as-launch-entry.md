...
当前工具栏点击由 ADR 0011 定义的 `chrome.action.onClicked` 打开独立 `save.html` 保存小窗口；完整 `index.html` 工作台由保存小窗口顶部外链按钮或“管理”Tab 打开。`popup.html` 保留为 fallback / dev entry。
...
- 普通新标签页保持浏览器默认页面。
- 用户需要点击或固定扩展工具栏图标来打开 Bookmark Visualizer 保存小窗口。
- 快速保存由独立保存小窗口承载，完整管理仍然保留完整页面空间，适合拖拽、搜索、行内编辑和操作日志。
