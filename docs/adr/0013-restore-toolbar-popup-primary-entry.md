...
当前产品优先级回到“点击工具栏图标即可打开稳定、可验证、功能完整的 Save / Manage / Settings popup”。旧 overlay / save-window 代码不在本轮删除，以降低回退风险，并留给后续 cleanup run 统一处理。
...
- `save.html`、Save Overlay 和 legacy command helper 暂时留在代码库中，但不是当前 toolbar 主路径。
...
- 当前入口验证命令为 `npm run verify:popup-entry`；旧 `verify:save-window-entry` 仅作为兼容 alias。
