...
- 点击浏览器工具栏图标：打开 Bookmark Visualizer popup，并默认进入当前网页保存入口。
- 点击浏览器新标签页按钮：默认保持浏览器原生 New Tab；当 popup 设置中开启“绑定新标签页”后，跳转到 Bookmark Visualizer New Tab Portal。
...
- 快捷操作只包含打开管理页、新建书签、导入 HTML 和自定义布局；保存当前标签页仍属于 popup 保存入口。
...
- 当前主路径是在普通网页点击扩展工具栏图标，打开浏览器标准 popup。
- popup 顶部包含“保存 / 管理 / 设置”三个 Tab，默认选中“保存”。
- popup 顶部展示 Logo、`我的书签`、`Bookmark Visualizer` 和打开完整工作台按钮。
- “管理 / 设置”Tab 第一阶段只提供打开完整工作台、快捷键说明和基础设置占位。
- popup 底部保存操作区只在“保存”Tab 显示；“管理 / 设置”Tab 不复用保存按钮。
- `Ctrl + S` 快捷键保存路线暂停；当前不默认注入全局网页 listener，也不依赖 Chrome Keyboard Shortcuts 的 `Ctrl + S` command 分配。
- 默认命令快捷键仍为 `Ctrl + Shift + S`，macOS 为 `Command + Shift + S`，用于保留的 content dialog 入口。
- 用户仍可打开浏览器扩展快捷键设置页，自定义 `Ctrl + Shift + S` 这一低权限命令入口。
- 保留命令在普通网页中触发居中保存浮框；popup 路径不覆盖整个屏幕。
- 快捷保存浮框标题区不显示固定快捷键文案，避免和用户在浏览器中自定义的快捷键不一致。
- 快捷保存浮框可通过标题区或空白区域拖动；按钮、输入框、备注框、文件夹菜单和预览图不触发拖动。
- popup 或保留浮框自动填充当前页面 URL、标题和候选预览图片。
- 用户可以编辑标题、复制只读 URL、填写备注，并通过搜索、最近使用或管理入口选择保存位置。
- popup 保存位置区显示完整当前保存路径，底部保存按钮显示 `保存到 {文件夹名}`。
- popup 保存位置路径文本是只读展示，不作为跳转入口，不触发完整管理页或保存位置菜单。
- popup 保存位置右侧箭头是保存位置级联菜单的唯一触发器；点击或键盘触发箭头时，以 popup 文档级 floating layer 从浏览器书签根级打开全局文件夹菜单。
- popup 保存位置级联菜单打开时，当前保存路径的各级文件夹只高亮不自动展开，最终保存文件夹显示“当前位置”；用户悬浮或键盘聚焦某个有子级的文件夹后才展开下一层，点击任意可保存文件夹后更新保存位置并关闭菜单。
- 文件夹搜索匹配文件夹名称和完整路径，结果按标题精确、标题前缀、标题包含、路径包含的顺序排序，并只在标题匹配足够明确时显示“最佳匹配”。
- 文件夹搜索框内按 `Esc` 时，有搜索词先清空搜索，搜索词为空时再允许关闭 popup。
- 最近使用文件夹在保存成功或新建文件夹成功后更新；保存位置面板默认展示 3 个可用目标，展开后最多展示 7 个，底层最近位置状态保留更多候选以支撑展开态。
- popup 通过右侧箭头提供轻量全局级联选择；菜单和子菜单可覆盖 popup 内容区并在 popup 视口内做碰撞检测，需要复杂位置管理时仍从“管理位置”进入完整工作台。
- popup 支持在当前保存位置下新建文件夹，也支持从级联菜单选择目标父级后回到主面板行内新建；创建成功后选中新文件夹作为保存位置。
- popup 预览图对 favicon、logo 和 icon 类图片使用完整展示策略，避免被封面裁切；普通大图仍可作为封面图展示。
- 保留 content dialog 的浏览面板和多级菜单能力可继续用于 `Ctrl + Shift + S` 入口。
...
- 点击 Cancel 或保存成功后关闭 popup；保留 content dialog 支持 Close、Escape 和保存成功关闭。
- 同一页面中关闭 popup 后再次点击工具栏图标，应重新打开 popup。
- 在 `chrome://`、扩展商店等受保护页面打开 popup 时，保存按钮禁用并显示当前页面不支持保存。
