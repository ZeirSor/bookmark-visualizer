...
截至 2026-05-08，当前代码已经完成 Toolbar Popup 保存入口、完整管理页三栏工作台、New Tab 可选入口、Quick Save 命令浮框、书签树浏览、书签卡片、标题 / URL 搜索、拖拽移动与重排、书签新建 / 编辑 / 移动 / 删除、文件夹新建 / 重命名 / 移动、最近文件夹、备注编辑、主题与卡片尺寸设置、侧栏宽度设置和会话内操作日志。下面的需求同时保留产品目标和当前实现差异。

## 功能需求

### 扩展入口

- 工具栏图标是当前主入口之一，但点击后打开的是标准 Toolbar Popup：`public/manifest.json` → `action.default_popup = popup.html`。
- Popup 顶部包含“保存 / 管理 / 设置”三个 Tab；默认打开 Tab 由 `settings.popupDefaultOpenTab` 控制。
- 完整管理页 `index.html` 通过 Popup 的管理入口、设置入口或其他深链接打开，不是工具栏图标的直接默认页面。
- 扩展默认不接管浏览器默认新标签页。
- 用户可以在 Popup 设置中开启“绑定新标签页”，开启后浏览器新标签页会由 service worker 条件重定向到 `newtab.html`；关闭后保留浏览器默认新标签页。
- New Tab Portal 是搜索、常用网站启动和轻量书签分组页，不是完整管理页缩小版。
- Quick Save 键盘入口保留为扩展命令 `open-quick-save`：默认 `Ctrl+Shift+S` / macOS `Command+Shift+S`。当前不默认注册全站 `Ctrl+S` content script listener。
...
- 搜索结果以卡片形式展示。
- 结果展示所在文件夹路径。
- 管理页当前显示排序、筛选、有备注、未读和收藏等命令栏入口；除批量选择外，其余结构化筛选和排序仍为禁用占位，不写入假状态。
