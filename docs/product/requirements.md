...
截至 2026-05-09，当前代码已经完成独立保存小窗口主入口、完整管理页三栏工作台、New Tab 可选入口、保留的 Quick Save 内容脚本浮框、书签树浏览、书签卡片、标题 / URL 搜索、拖拽移动与重排、书签新建 / 编辑 / 移动 / 删除、文件夹新建 / 重命名 / 移动、最近文件夹、备注编辑、主题与卡片尺寸设置、侧栏宽度设置和会话内操作日志。下面的需求同时保留产品目标和当前实现差异。

## 功能需求

### 扩展入口

- 工具栏图标是当前主保存入口；点击后由 service worker 打开或聚焦独立 `save.html` 小窗口。
- 保存小窗口顶部包含“保存 / 管理 / 设置”三个 Tab；默认打开 Tab 由 `settings.popupDefaultOpenTab` 控制。
- 完整管理页 `index.html` 通过保存小窗口的管理入口、设置入口或其他深链接打开，不是工具栏图标的直接默认页面。
- 扩展默认不接管浏览器默认新标签页。
...
- Quick Save 键盘入口保留为扩展命令 `open-quick-save`：默认 `Ctrl+Shift+S` / macOS `Command+Shift+S`，当前打开独立保存小窗口。当前不默认注册全站 `Ctrl+S` content script listener。
...
- 用户可以手动编辑备注。
- 用户可以在普通网页点击扩展工具栏图标，打开独立保存小窗口的“保存”Tab，并保存当前网页为浏览器原生书签。
- 保存窗口表单自动填充当前 URL、标题和候选预览图片，允许用户编辑标题、复制只读 URL、填写备注并选择保存文件夹。
- popup 顶部保留“保存 / 管理 / 设置”三个 Tab；管理 Tab 提供完整管理页入口、最近保存和最近使用文件夹，设置 Tab 提供快捷键说明、默认保存位置、保存行为和界面偏好等常用设置。
- 保存窗口位置区支持完整路径确认、右侧箭头触发的全局文件夹级联菜单、按名称或路径搜索、最近使用文件夹、管理位置入口和当前层级新建文件夹；路径文本本身只读展示，不作为跳转入口，最近使用默认展示 3 个并可展开到最多 7 个。
...
- 操作必须真实同步到浏览器原生书签。
- 插件不读取网页正文，除非用户主动触发摘要抓取；当前保存窗口只在用户点击工具栏图标或触发保存命令后读取 source tab 标题、URL 和候选图片。浏览器内部页面不执行脚本注入。
- 大量书签下搜索和树渲染应保持可用。
