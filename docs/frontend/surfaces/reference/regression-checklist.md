...
## Popup Save / Page Shortcut 回归

- Toolbar icon opens `popup.html`.
- `_execute_action` opens the same popup.
- Popup Save Tab can save the current tab title and URL.
- Popup location picker supports search, recent folders and creating a folder.
- Optional page Ctrl/Cmd+S bridge opens the popup only when enabled and permitted.
- Page shortcut bridge does not render a content-script dialog.
...
2. Popup 保存 Tab。
3. New Tab 首屏。
4. Popup Settings 默认保存位置和 New Tab 首屏不受影响。
