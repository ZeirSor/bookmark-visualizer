...
| storage key 只写了旧的三个 key，且把 `bookmarkVisualizerQuickSaveUiState` 写成主 key | `data/storage.md`、`frontend/surfaces/shared/data-storage-chrome-api.md` |
...
| New Tab 建议项 selector 写成 `.nt-search-row` | `frontend/surfaces/newtab/layout-search-shortcuts.md`、`reference/code-navigation-index.md` |
| New Tab 三种布局模式被写成完整对称 CSS 体系 | `frontend/surfaces/newtab/css-maintenance.md` |
| `src/popup/tabs/SettingsTab.tsx` 被误读为现有文件 | `frontend/surfaces/popup/settings-manage-tab.md` |
| background 文件导航过粗，New Tab redirect 位置不清晰 | `architecture/overview.md`、`frontend/surfaces/reference/code-navigation-index.md` |
