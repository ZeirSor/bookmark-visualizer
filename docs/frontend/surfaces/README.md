...
| Legacy 保存页 | `save.html` | `src/save-window/SaveWindowApp.tsx` → `src/popup/PopupApp.tsx` | `src/save-window/main.tsx` → `src/styles/tokens.css` + `src/popup/styles.css` + `src/save-window/styles.css` | 保留的保存页入口 |
